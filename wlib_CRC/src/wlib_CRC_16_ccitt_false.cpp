...
namespace wlib::crc
{
  ...
  CRC_16_ccitt_false::used_type CRC_16_ccitt_false::operator()(std::byte const* beg, std::byte const* end) noexcept
  {
    while (beg < end)
    {
      this->m_crc = static_cast<uint16_t>((this->m_crc << 8) ^ table[static_cast<uint8_t>(((this->m_crc >> 8) & 0xFF) ^ static_cast<uint8_t>(*beg))]);
      ++beg;
    }
